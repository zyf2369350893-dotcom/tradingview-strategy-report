...
def fetch_coinbase_crypto(symbol: str, timeframe: str, bars: int) -> pd.DataFrame:
    instrument = official_crypto_id(symbol)
    needed_days = bars + 10 if timeframe == "daily" else bars * (32 if timeframe == "monthly" else 7) + 14
...
def fetch_sse_etf(symbol: str, timeframe: str, bars: int) -> pd.DataFrame:
    ...
    history_days = 365 * (3 if timeframe == "daily" else (20 if timeframe == "monthly" else 12))
...
def fetch_ohlcv(yahoo: str, timeframe: str, bars: int, market: str) -> pd.DataFrame:
    ...
    interval = "1d"
    periods = ["3y", "2y", "1y", "6mo", "3mo"] if timeframe == "daily" else ["20y", "15y", "10y", "5y", "3y", "1y", "6mo"] if timeframe == "monthly" else ["10y", "5y", "3y", "1y", "6mo"]
    last_error: Exception | None = None
